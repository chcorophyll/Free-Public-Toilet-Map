...
export const DetailsPanel: React.FC<DetailsPanelProps> = ({
  ...
  const handleCopyCoords = () => {
    if (!toilet) return;
    const coordsString = `${toilet.location.coordinates[1].toFixed(
      5
  ...
  // 响应式布局和动画的 CSS 类
  const panelClasses = `fixed bg-[#1E1E1E] transform transition-transform duration-500 ease-in-out z-30 ${
    toilet
      ? "translate-y-0 md:translate-x-0"
      : "translate-y-full md:-translate-x-full"
  } bottom-0 left-0 right-0 rounded-t-2xl border-t border-gray-700 md:top-0 md:h-screen md:w-96 md:rounded-none md:border-t-0 md:border-r`;
      ...
      {toilet && (
        <div className="flex flex-col h-full text-white">
          ...
          <div className="flex-shrink-0 p-6 flex justify-between items-start">
            ...
            <button
              onClick={onClose}
              className="p-2 -m-2 rounded-full hover:bg-gray-700"
            >
              {" "}
              <CloseIcon />{" "}
            </button>
              ...
              <div>
                ...
                <div className="p-4 bg-gray-800 rounded-lg space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <p className="text-gray-400">{`${toilet.location.coordinates[1].toFixed(
                      5
                    )}, ${toilet.location.coordinates[0].toFixed(5)}`}</p>
                    <button
          ...
          <div className="flex-shrink-0 p-6 border-t border-gray-800">
            <button
              ...
              className="w-full p-4 bg-[#CEFF00] text-black text-lg font-bold rounded-lg hover:opacity-90"
            >
              开始导航
            </button>
          </div>
