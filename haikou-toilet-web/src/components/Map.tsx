...
const defaultIcon = new L.Icon({ iconUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon.png', iconRetinaUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon-2x.png', shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png', iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34], shadowSize: [41, 41] });
const selectedIcon = new L.Icon({ iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-yellow.png', shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png', iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34], shadowSize: [41, 41] });
const userLocationIcon = new L.divIcon({ html: `<div class="relative flex h-4 w-4"><div class="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></div><div class="relative inline-flex rounded-full h-4 w-4 bg-sky-500 border-2 border-white"></div></div>`, className: 'bg-transparent border-0', iconSize: [16, 16], iconAnchor: [8, 8], });

// This helper component now creates the bounds object internally.
const FitBounds = ({ points, panelPadding }: { points: Array<[number, number]> | null, panelPadding: [number, number] }) => {
  const map = useMap();
  useEffect(() => {
...
const Map: React.FC<MapProps> = ({ toilets, center, userLocation, selectedToiletId, pointsToFit, panelPadding, onMarkerClick }) => {
  return (
    <MapContainer center={center} zoom={14} style={{ height: '100%', width: '100%' }}>
      ...
      />
      {userLocation && ( <Marker position={userLocation} icon={userLocationIcon}><Popup>你在这里</Popup></Marker> )}
      {toilets.map(toilet => {
        ...
        return (
          ...
          >
            <Popup><div className="text-black"><h3 className="font-bold">{toilet.name}</h3><p>{toilet.address}</p></div></Popup>
          </Marker>
        )
      })}
    </MapContainer>
